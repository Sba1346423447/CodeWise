...
- **Tool-Augmented 工具扩展**：隔离沙箱代码执行、自动化验证、静态检查、联网检索，四类工具可插拔扩展
...
```
databox/
├── config/                     # 提示词 / 主配置 / 工具定义（非代码人员可调优）
│   ├── prompts.yaml            # 各阶段提示词模板（ReAct / 反思 / 优化）
│   ├── settings.yaml           # 模型名、最大反思次数、沙箱路径、超时时间
│   └── tools.yaml              # 工具定义：名称、描述、参数 schema
├── backend/                    # FastAPI 后端
│   ├── app/
...
- **配置与代码解耦**：提示词 / 参数 / 工具定义统一 YAML 管理，非代码人员可调优
